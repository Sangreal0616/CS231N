...
def sample_noise(batch_size, dim, seed=None):
   ...
   # Generate uniform random noise in range [0, 1]
...
def build_dc_classifier(batch_size):
    ...
    model = nn.Sequential(
        ...
        nn.LeakyReLU(negative_slope=0.01),  # Leaky ReLU(alpha=0.01)
        ...
        # Fully Connected: output size 4 x 4 x 64
...
def build_dc_generator(noise_dim=NOISE_DIM):
    ...
    model = nn.Sequential(
        ...
        # Reshape into (128, 7, 7) manually
        nn.Unflatten(1, (128, 7, 7)),

        # ConvTranspose2d: 64 filters, 4x4, stride 2, 'same' padding
        nn.ConvTranspose2d(in_channels=128, out_channels=64, kernel_size=4, stride=2, padding=1),
        nn.ReLU(),
        nn.BatchNorm2d(64),

        # ConvTranspose2d: 1 filter, 4x4, stride 2, 'same' padding
        nn.ConvTranspose2d(in_channels=64, out_channels=1, kernel_size=4, stride=2, padding=1),
        nn.Tanh(),

        # Flatten the output back to a 784 vector
        nn.Flatten()
    )
