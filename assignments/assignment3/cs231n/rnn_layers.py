...
def rnn_step_backward(dnext_h, cache):
    ...
    dtanh = (1 - next_h**2) * dnext_h  # tanh'(x) * dnext_h

    # 역전파 계산
    dx = np.dot(dtanh, Wx.T)  # 입력 기울기
    dprev_h = np.dot(dtanh, Wh.T)  # 이전 h 상태 기울기
    dWx = np.dot(x.T, dtanh)  
    dWh = np.dot(prev_h.T, dtanh)  
    db = np.sum(dtanh, axis=0)  # 편향 기울기 
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
        # x[:, t, :]는 현재 타임스텝 t의 입력 데이터
        next_h, step_cache = rnn_step_forward(x[:, t, :], prev_h, Wx, Wh, b)
        h[:, t, :] = next_h   # 현재 타임스텝의 은닉 상태 저장
        cache.append(step_cache)  # 현재 타임스텝의 캐시 저장
        prev_h = next_h  # 다음 타임스텝으로 은닉 상태 전달
...
def rnn_backward(dh, cache):
    ...
    N, T, H = dh.shape
    x, _, Wx, Wh, _, _ = cache[0]  # 첫 번째 타임스텝의 캐시에서 x와 Wx 가져오기
    D = x.shape[1]
    ...
    dWh = np.zeros((H, H))
    db = np.zeros(H)
    dprev_h = np.zeros((N, H))  # 이전 타임스텝으로 전파될 은닉 상태 기울기

    for t in reversed(range(T)):
        # 현재 타임스텝의 dh와 이전 타임스텝에서 넘어온 dprev_h를 합산
        dcurrent_h = dh[:, t, :] + dprev_h

        # rnn_step_backward 호출
...
def word_embedding_backward(dout, cache):
    ...
    #np.add.at(array, indices, values)
      # array: 값을 추가할 배열.
      # indices: 값을 더할 위치(인덱스). 단일 인덱스 또는 배열로 지정.
      # values: 더할 값. 인덱스와 크기가 같아야 함.
    np.add.at(dW,x,dout)
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    # Split activation vector into gates: input, forget, output, and gate
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    # Gradients of the output gate
    do = dnext_h * np.tanh(next_c)
    dnext_c_total = dnext_c + dnext_h * o * (1 - np.tanh(next_c) ** 2)

    # Gradients of the cell state
    ...
    dai = di * i * (1 - i)       # Derivative of sigmoid for input gate
    daf = df * f * (1 - f)       # Derivative of sigmoid for forget gate
    dao = do * o * (1 - o)       # Derivative of sigmoid for output gate
    dag = dg * (1 - g ** 2)      # Derivative of tanh for gate gate

    # Concatenate the gradients of the gates
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    # 각 타임스텝에 대해 lstm_step_forward 수행
    for t in range(T):
...
def lstm_backward(dh, cache):
    ...
    N, T, H = dh.shape
    D = cache[0][0].shape[1]  # x의 shape (N, D)에서 D를 가져옴
    ...
    for t in reversed(range(T)):
        step_cache = cache[t]
        dcurrent_h = dh[:, t, :] + dprev_h  # dh와 dprev_h를 합산
        dx_t, dprev_h, dprev_c, dWx_t, dWh_t, db_t = lstm_step_backward(dcurrent_h, dprev_c, step_cache)
        dx[:, t, :] = dx_t
        dWx += dWx_t
        dWh += dWh_t
        db += db_t
