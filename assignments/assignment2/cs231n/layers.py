...
def batchnorm_backward(dout, cache):
    ...
    dx_hat = dout * gamma
    dlvar = np.sum(dx_hat * (x - sample_mean) * -0.5 * (sample_var + eps)**-1.5, axis = 0)
    dlmean = np.sum(dx_hat * -1 / np.sqrt(sample_var + eps) , axis = 0) 
    + dlvar * np.sum(-2 * (x - sample_mean), axis = 0) / N
    dx = dx_hat * 1 / np.sqrt(sample_var + eps) + dlvar * 2 * (x - sample_mean) / N + dlmean / N
...
def batchnorm_backward_alt(dout, cache):
    ...
    dmean = np.sum(dx_hat * -1 / np.sqrt(sample_var + eps), axis=0) + dvar * np.sum(-2 * (x - sample_mean), axis=0) / N
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    # 컨볼루션 연산
    for n in range(N):  # 데이터 포인트별
        for f in range(F):  # 필터별
            for i in range(H_out):  # 출력 높이별
                for j in range(W_out):  # 출력 너비별
                    # 입력에서 슬라이딩 윈도우를 가져오기
                    i_start, i_end = i * stride, i * stride + HH
                    j_start, j_end = j * stride, j * stride + WW
