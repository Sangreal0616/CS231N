...
def affine_backward(dout, cache):
    ...
    # 기울기 계산
    dw = x_reshaped.T.dot(dout)        # w에 대한 기울기
    db = dout.sum(axis=0)              # b에 대한 기울기
    dx_reshaped = dout.dot(w.T)        # x에 대한 기울기 (reshaped 상태)
    dx = dx_reshaped.reshape(x.shape)  # x를 원래 형태로 복원
